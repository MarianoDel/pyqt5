...
from ui_stretcher40 import Ui_Dialog
...
class Dialog(QDialog):
    ...
    def __init__(self, debug_bool, ser_instance, treat_instance, parent=None):
        ...
        ## set first config on antenna button
        self.antennas_connected.SetActiveStatus('ch1', True)
        self.antennas_connected.SetActiveStatus('ch2', True)
        self.antennas_connected.SetActiveStatus('ch3', True)        
    ...
    def ChannelChange (self):
        ...
        if sender.objectName() == 'ch1Button':
            if self.antennas_connected.GetActive('ch1') == False:
                self.ui.ch1Button.setStyleSheet(self.style.ch_enable_no_align)
                self.antennas_connected.SetActiveStatus('ch1', True)
            else:
                self.ui.ch1Button.setStyleSheet(self.style.ch_disable_no_align)
                self.antennas_connected.SetActiveStatus('ch1', False)

        if sender.objectName() == 'ch2Button':
            if self.antennas_connected.GetActive('ch2') == False:
                self.ui.ch2Button.setStyleSheet(self.style.ch_enable_no_align)
                self.antennas_connected.SetActiveStatus('ch2', True)
            else:
                self.ui.ch2Button.setStyleSheet(self.style.ch_disable_no_align)
                self.antennas_connected.SetActiveStatus('ch2', False)

        if sender.objectName() == 'ch3Button':
            if self.antennas_connected.GetActive('ch3') == False:
                self.ui.ch3Button.setStyleSheet(self.style.ch_enable_no_align)
                self.antennas_connected.SetActiveStatus('ch3', True)
            else:
                self.ui.ch3Button.setStyleSheet(self.style.ch_disable_no_align)
                self.antennas_connected.SetActiveStatus('ch3', False)
    ...
    #     if sender.objectName() == 'ch4Button':
    #         pass
    """ end of function for tests, generates the required antenna strings """
    ...
    def CheckCompleteConf (self):
        ...
        if self.antennas_connected.GetActive('ch1') != False or \
           ...
           self.antennas_connected.GetActive('ch3') != False:
            channels_in = True
